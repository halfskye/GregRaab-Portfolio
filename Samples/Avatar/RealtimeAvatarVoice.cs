...
namespace Avatar {
    ...
    public class RealtimeAvatarVoice : RealtimeComponent<RealtimeAvatarVoiceModel> {
        ...
        private AvatarAudioProcessor.AvatarAudioProcessor _avatarAudioProcessor = null;
        ...
        private void Awake()
        {
            _avatarAudioProcessor = this.GetComponent<AvatarAudioProcessor.AvatarAudioProcessor>();
        }
        ...
        private void ConnectLocalAudioStream() {
            ...
            if (foundOculusMicrophoneDevice) {
                ...
                _microphoneSampleRate = 48000;
                _microphoneChannels   = 1;
            } else if (Normal.Realtime.Native.Microphone.PlatformSupported()) {
                _nativeMicrophoneDevice = new Normal.Realtime.Native.Microphone();
        ...
        void SendMicrophoneData() {
            ...
            if (didGetAudioData && _avatarAudioProcessor != null)
            {
                DebugLog("[RealtimeAvatarVoice] Testing if didGetAudioData but no frame data...");
                if (_microphoneFrameData.Any(x => !Mathf.Approximately(x, 0f)))
                {
                    DebugLog("[RealtimeAvatarVoice] Processing Microphone Frame Data...");
                    _avatarAudioProcessor.Process(_microphoneFrameData);
                }
            }
        ...
        private static bool USE_DEBUG_LOGS = false;
        private void DebugLog(string message)
        {
            #if UNITY_EDITOR
            if (USE_DEBUG_LOGS)
            {
                Debug.Log($"[RealtimeAvatarVoice] {message}", this);
            }
            #endif
        }
    }
